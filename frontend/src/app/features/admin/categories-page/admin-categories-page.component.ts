    ...
    <section class="space-y-6">
      <div class="app-surface p-6 sm:p-8">
        <app-page-header
          ...
          description="Create, update, and organize catalog categories from one dedicated admin view."
        />
      </div>
...
export class AdminCategoriesPageComponent implements OnInit {
  ...
  loadCategories(): void {
    ...
    this.adminService.getCategoryTree().subscribe({
      next: (res) => {
  ...
  updateCategory(categoryId: string, payload: { name: string; description: string; parentCategory: string; isActive: boolean }): void {
    ...
    this.adminService.updateCategory(categoryId, {
  ...
  totalCategoryCount(categories: CategoryRecord[]): number {
