...
import { AdminProductRecord, CategoryRecord, ToastType } from '../../../core/models/admin.models';
...
export class AdminProductsPageComponent implements OnInit {
  ...
  submitProduct(): void {
    ...
    if (!this.form.productName.trim() || !this.form.productDescription.trim() || !this.form.category) {
      this.createError = 'Product name, description, and category are required.';
      return;
    }
  ...
  openProductDetails(product: AdminProductRecord): void {
    this.router.navigate(['/admin/products', product._id]);
  }

  variantStockKey(product: AdminProductRecord, variant: any, index: number): string {
    return `${product._id}-${variant?._id || index}`;
  }

  stockBusyKey(product: AdminProductRecord, variant: any, index: number): string {
    return `stock-${this.variantStockKey(product, variant, index)}`;
  }

  stockAmount(product: AdminProductRecord, variant: any, index: number): number {
    const key = this.variantStockKey(product, variant, index);
  ...
  updateStockAmount(product: AdminProductRecord, variant: any, index: number, value: number | string): void {
    const key = this.variantStockKey(product, variant, index);
    const amount = Math.max(1, Number(value) || 1);
    this.stockAdjustments[key] = amount;
  }
  ...
  adjustVariantStock(
    ...
    if (!variant?._id) {
      this.setMessage(product._id, 'error', 'Variant not found.');
      return;
    }

    const amount = Math.max(1, Number(this.stockAmount(product, variant, index)) || 1);
    const delta = direction === 'add' ? amount : -amount;
    const busyKey = this.stockBusyKey(product, variant, index);

    this.setBusy(busyKey, true);
    this.adminService.adjustVariantStock(product._id, variant._id, delta).subscribe({
      next: (res) => {
        this.setBusy(busyKey, false);
        if (!res?.success) {
          this.setMessage(product._id, 'error', res?.message || 'Unable to update stock.');
          return;
        }

        if (res?.data) {
          const updatedProduct = res.data as AdminProductRecord;
          this.products = this.products.map((entry) =>
            entry._id === updatedProduct._id ? updatedProduct : entry
          );
        }

        this.setMessage(product._id, 'success', res?.message || 'Stock updated successfully.');
      },
      error: (err) => {
        this.setBusy(busyKey, false);
        this.setMessage(product._id, 'error', err.error?.message || 'Unable to update stock.');
      }
    });
  }
