...
@Component({
  ...
  template: `
    <aside>
      <div
        *ngIf="mobileOpen"
        class="fixed inset-0 z-50 bg-slate-950/45 backdrop-blur-[2px] lg:hidden"
        (click)="closeMobile.emit()"
      >
        <div
          class="absolute left-0 top-0 h-full w-[min(86vw,20rem)] overflow-y-auto bg-white shadow-[0_24px_60px_rgba(15,23,42,0.16)]"
          (click)="$event.stopPropagation()"
        >
          <div class="flex items-start justify-between border-b border-slate-200 px-4 py-4">
            <div class="min-w-0">
              <p class="vendor-stat-label">Vendor navigation</p>
              <p class="mt-2 text-lg font-black text-slate-900">Menu</p>
            </div>
            <button
              type="button"
              class="rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-black uppercase tracking-[0.16em] text-slate-700"
              (click)="closeMobile.emit()"
            >
              Close
            </button>
      ...
      <div class="hidden lg:block">
        <div class="glass-card flex min-w-0 flex-col p-3 md:p-4 lg:min-h-[calc(100vh-6rem)]">
          <nav class="mt-2 space-y-2.5">
          @for (item of sidebarItems; track item.view) {
            @if (item.view !== 'shipments' || showShipments) {
              <a
                [routerLink]="item.link"
                class="flex w-full min-w-0 rounded-xl border px-3 py-2.5 text-left transition-all"
                [ngClass]="activeView === item.view ? item.activeClasses : item.inactiveClasses"
              >
