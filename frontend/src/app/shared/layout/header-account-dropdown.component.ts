...
type HeaderDropdownTheme = 'customer' | 'storeOwner' | 'admin';
...
export class HeaderAccountDropdownComponent {
  ...
  desktopTriggerClasses(): string {
    const themeClasses: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: 'group inline-flex items-center gap-3 rounded-full border border-emerald-100 bg-white/95 px-2.5 py-2 pr-4 text-left shadow-[0_16px_40px_rgba(16,185,129,0.12)] transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_20px_44px_rgba(16,185,129,0.18)]',
  ...
  mobileTriggerClasses(): string {
    const themeClasses: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: 'inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-white px-2.5 py-2 shadow-[0_16px_32px_rgba(16,185,129,0.12)] transition-all duration-200 hover:border-emerald-200',
  ...
  avatarClasses(): string {
    if (this.desktop) {
      const desktopClasses: Record<HeaderDropdownTheme, string> = {
        ...
        storeOwner: 'flex h-11 w-11 items-center justify-center rounded-full bg-[linear-gradient(135deg,#059669_0%,#10b981_55%,#34d399_100%)] text-sm font-black uppercase tracking-[0.12em] text-white ring-2 ring-white',
    ...
    const mobileClasses: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: 'flex h-10 w-10 items-center justify-center rounded-full bg-[linear-gradient(135deg,#059669_0%,#10b981_55%,#34d399_100%)] text-sm font-black uppercase tracking-[0.12em] text-white',
  ...
  heroClasses(): string {
    const tone: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: 'rounded-[1.25rem] bg-[linear-gradient(135deg,rgba(236,253,245,0.95),rgba(240,253,250,0.95))] p-4',
  ...
  heroAvatarClasses(): string {
    ...
    const tone: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: `flex ${size} items-center justify-center rounded-full bg-[linear-gradient(135deg,#059669_0%,#10b981_55%,#34d399_100%)] text-sm font-black uppercase tracking-[0.12em] text-white`,
  ...
  subtitleClass(): string {
    const tone: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: 'uppercase tracking-[0.18em] text-emerald-600',
  ...
  chevronOpenClass(): string {
    const tone: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: 'rotate-180 text-emerald-700',
  ...
  panelClasses(): string {
    ...
    const tone: Record<HeaderDropdownTheme, string> = {
      ...
      storeOwner: `absolute right-0 top-[calc(100%+12px)] ${width} origin-top-right rounded-[1.5rem] border border-emerald-100 bg-white/95 p-3 shadow-[0_24px_60px_rgba(15,23,42,0.16)] backdrop-blur transition-all duration-200`,
  ...
  arrowClass(tone: 'default' | 'accent' | 'danger'): string {
    const classes: Record<string, string> = {
      ...
      accent: this.theme === 'storeOwner' ? 'text-emerald-300' : 'text-indigo-300',
